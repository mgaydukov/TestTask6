...
public class GlobalExceptionHandler {
    ...
    // Обработка исключений SaleNotFoundException
    @ExceptionHandler(SaleNotFoundException.class)
    public ResponseEntity<ErrorDetails> handleSaleNotFoundException(SaleNotFoundException ex, WebRequest request) {
        ErrorDetails errorDetails = new ErrorDetails(
                LocalDateTime.now(),
                ex.getMessage(),  // Сообщение из SaleNotFoundException
                request.getDescription(false)
        );
        return new ResponseEntity<>(errorDetails, HttpStatus.NOT_FOUND);
    }

    // Обработка исключений SupplyNotFoundException
    @ExceptionHandler(SupplyNotFoundException.class)
    public ResponseEntity<ErrorDetails> handleSupplyNotFoundException(SupplyNotFoundException ex, WebRequest request) {
        ErrorDetails errorDetails = new ErrorDetails(
                LocalDateTime.now(),
                ex.getMessage(),  // Сообщение из SupplyNotFoundException
                request.getDescription(false)
        );
        return new ResponseEntity<>(errorDetails, HttpStatus.NOT_FOUND);
    }
